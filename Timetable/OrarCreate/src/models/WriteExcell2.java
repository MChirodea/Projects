...
public class WriteExcell2 {
	...
	public void addTest12(String zi, IntervalOrar inter, Boolean bool, Sala sal, CadruDidactic cad) {
		if (test.get(zi) == null) {
			...
			tempS.put(sal.getNumeSala(), temp1);
			...
			tempSa.put(sal.getNumeSala(), temp5);
			...
			if (test.get(zi).get(inter) == null) {
				...
				temp11.add(cad);
				LinkedHashMap<String, LinkedHashSet<CadruDidactic>> tempSa = new LinkedHashMap<String, LinkedHashSet<CadruDidactic>>();
				tempSa.put(sal.getNumeSala(), temp11);
				LinkedHashMap<Boolean, LinkedHashMap<String, LinkedHashSet<CadruDidactic>>> tempG = new LinkedHashMap<Boolean, LinkedHashMap<String, LinkedHashSet<CadruDidactic>>>();
				tempG.put(bool, tempSa);
				...
				if (test.get(zi).get(inter).get(bool) == null) {
					...
					if (test.get(zi).get(inter).get(bool).get(sal.getNumeSala()) == null) {
						LinkedHashSet<Sala> temp1 = new LinkedHashSet<Sala>();
						temp1.add(sal);
						test.get(zi).get(inter).get(bool).put(sal.getNumeSala(), temp1);
						LinkedHashSet<CadruDidactic> temp11 = new LinkedHashSet<CadruDidactic>();
						temp11.add(cad);
						test2.get(zi).get(inter).get(bool).put(sal.getNumeSala(), temp11);
					} else {

						if (test.get(zi).get(inter).get(bool).get(sal.getNumeSala()).contains(sal) == false) {
							test.get(zi).get(inter).get(bool).get(sal.getNumeSala()).add(sal);
							test2.get(zi).get(inter).get(bool).get(sal.getNumeSala()).add(cad);
						}
					}
				}
			}
	...
	public void writeDataRiseI(HSSFSheet stuff, mainControl mn, String name) {
			...
			mn.getOrarSala().get(item).keySet().stream().forEach(item2 -> {
				mn.getOrarSala().get(item).get(item2).keySet().stream().forEach(item3 -> {
					mn.getOrarSala().get(item).get(item2).get(item3).stream().forEach(item4 -> {
						if (item4.getNumeSala().equals(name)) {
							flag = 0;
							try {
		...
		test.keySet().stream().forEach(item -> {
			test.get(item).keySet().stream().forEach(item2 -> {
				test.get(item).get(item2).keySet().stream().forEach(item3 -> {
					if (test.get(item).get(item2).get(item3).keySet().contains(name)) {
						test.get(item).get(item2).get(item3).get(name).stream().forEach(item5 -> {
							test2.get(item).get(item2).get(item3).get(name).stream().forEach(item6 -> {
								int flag1 = 0;
								for (int i = 0; i < 10; i++) {
							...
							if (flag == 1) {
								...
								if (!(prevInter.equals(item2) && prevDisciplina.equals(tempo.getDisciplina() )&& prevZi.equals(item))) {
									prevInter = item2;
									prevZi = item;
									...
									if (test.get(item).get(item2).get(!(item3)) != null) {
										if (!(stuff.getRow(count - 1).getCell(3).getStringCellValue()
												...
												&& stuff.getRow(count - 1).getCell(2).getStringCellValue()
														.equals(tempo.getNume())&& stuff.getRow(count - 1).getCell(0).getStringCellValue().equals(item))) {
												...
												if (stuff.getRow(count - 2).getCell(3).getStringCellValue()
													...
													if (item3) {
														if (tempo.getDisciplina().contains("curs")) {
															...
															cell3.setCellValue(tempo.getNume());
															cell3.setCellStyle(style6);
															cell4.setCellValue(tempo.getDisciplina());
															...
															for(int i = 0; i<10;i++) {
																if(tempo.getAn(i) != null) {
																	...
																	if(c != 0) {
																	cell5.setCellValue(cell5.getStringCellValue() +", " +tempo.getAn(i).getAnStudiu());
																	}else {
																		c++;
										...
										if (!(stuff.getRow(count - 1).getCell(3).getStringCellValue()
											...
											if (stuff.getRow(count - 2).getCell(3) != null) {
												if (stuff.getRow(count - 2).getCell(3).getStringCellValue()
														...
														&& stuff.getRow(count - 2).getCell(2).getStringCellValue()
																.equals(tempo.getNume()) && stuff.getRow(count - 2).getCell(0).getStringCellValue().equals(item)) {
													String temporary = stuff.getRow(count - 1).getCell(1)
															.getStringCellValue();
													stuff.getRow(count - 2).getCell(1)
															.setCellValue(temporary.split("(?<=-)")[0] + " "
		...
		if (stuff.getRow(7) != null) {
			...
			for (int i = 7; i < count; i++) {
				...
				stuff.getRow(i - 1).getCell(5).setCellValue(stuff.getRow(i - 1).getCell(5).getStringCellValue() + " - săpt. impară");
				stuff.getRow(i).getCell(3).setCellValue("/ " + stuff.getRow(i).getCell(3).getStringCellValue());
				stuff.getRow(i).getCell(5).setCellValue(stuff.getRow(i).getCell(5).getStringCellValue() +" săpt. pară");
					}else {
	...
	public void ExportHelper(HSSFSheet sheet1, mainControl mn, String sem) {
		...
		cell4.setCellValue("ORAR SALA " + sheet1.getSheetName());
		...
		cell63.setCellValue("CADRUL DIDACTIC");
...
public ArrayList<String> extractSala(mainControl mn, ArrayList<String> temp) {
		
		mn.getOrarSala().keySet().stream().forEach(item -> {
			mn.getOrarSala().get(item).keySet().stream().forEach(item2 -> {
				mn.getOrarSala().get(item).get(item2).keySet().stream().forEach(item3 -> {
					mn.getOrarSala().get(item).get(item2).get(item3).stream().forEach(item4 -> {
						if(!(temp.contains(item4.getNumeSala()))) {
							temp.add(item4.getNumeSala());
						}
					});
				});
			});
	...
	public void Export(mainControl mn, String sem) {
		...
		try {
			workbook.write(new FileOutputStream("orarSala.xls"));
